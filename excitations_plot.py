...
def create_sub_plots(axs, excitations, resolution, cycle_duration, title,
                     lower_dim, upper_dim):
    ...
    for i, act in enumerate(excitations):
        if i >= lower_dim and i < upper_dim:
            lines.append(axs.plot(time, act, color=lc[i],
                                  label=('Comp-%s' % (i+1))))
...
def calculate_excitation_violations(excitations, excite_stats, singular_values,
                                    num_frames, ignore_cycles=0):
...
def plot_kin_char_excitations(ax, data_file, num_frames, resolution, animate=False,
                              cycle_duration=1, lower_dim=0, upper_dim=28,
    ...
    # Plot Actions/Pose
    lines = create_sub_plots(ax, kin_extns, resolution, cycle_duration,
                             ("Kin-Char Excitations" if not sub_title else sub_title),
                             lower_dim, upper_dim)
    return lines, last_cycle


def plot_extracted_excitations(ax, data_file, basis_inv, num_frames, resolution,
                               cycle_duration, mean=None, lower_dim=0, upper_dim=28,
                               kin_extn_mat=None, diff_imitation=False, root_pos=False,
                               root_rot=False, axis_angle=False, excite_stats=None,
                               animate=False, ignore_cycles=0, offset=0,
                               dtype=np.float64):
    ...
    # Plot Actions/Pose
    lines = create_sub_plots(ax, extracted_euler_excits, resolution, cycle_duration,
                             ("Baseline Actions" if not sub_title else sub_title),
                             lower_dim, upper_dim)
    ...
    if diff_imitation and kin_extn_mat is not None:
        if last_cycle.shape == kin_extn_mat.shape:
            ...
            print("Mean Excitation Imitation Diff: ", mean_diff, " 1/e(-diff): ",
                  neg_exp)
    return lines


def plot_learned_excitations(ax, learned_excitation_file, num_frames, resolution,
                             cycle_duration, lower_dim=0, upper_dim=28,
                             excite_stats=None, animate=False, ignore_cycles=0, offset=0,
                             singular_values=None, dtype=np.float64):
    ...
    lines = create_sub_plots(ax, lrnd_excits, resolution, cycle_duration,
                            "Lower Dimension (ours) Actions", lower_dim,
                            upper_dim)

    if excite_stats is not None:
        scale_factor = 1000.0
        violations_mean, violations_mean_individual = \
            calculate_excitation_violations(learned_excitations, excite_stats,
                                            singular_values, num_frames,
                                            ignore_cycles)
...
def plot_reference_excitations(ax, ref_file, resolution, cycle_duration, lower_dim,
                               upper_dim, sub_title="Reference Motion"):
    ...
    ref_extns = np.hsplit(redu_ref_extn, redu_ref_extn.shape[1])

    # Plot reference excitations
    lns = create_sub_plots(ax, ref_extns, resolution, cycle_duration,
                           sub_title, lower_dim, upper_dim)
    return lns
...
def usage():
    print("Usage: excitations_plot.py [-a | --animate] \n"
          ...
          "                           [-i | --ignore_cycles] <no. of initial cycles to be ignored> \n"
          "                           [-k | --kin_excitation] <kinematic character excitation file> \n"
...
def main(argv):
    ...
    try:
        opts, args = getopt.getopt(argv, "h abdqn:c:e:i:k:l:o:p:r:u:",
                                   ["animate", "bound_violation", "diff_imitation",
                                    "quaternion", "action", "num_cycles", "excitation",
                                    "ignore_cycles", "kin_excitation", "lower",
                                    "offsets", "pose", "ref", "upper"])
    ...
    fig, axs = plt.subplots(num_plots, sharey=True, sharex=(False if animate and
                                                            num_cycles > 1 else True))
    ...
    def plot(i):
        ...
        with open(reduced_reference_files[0]) as f:
            data = json.load(f)

        # Extract resolution and cycle-duration from reduced reference file
        ...
        if bound_violation:
            try:
                ...
                print("ERROR: Flag '-b'|'--bound_violation' set, but reference ",
                      "motion file does not contain Excite_Min")
                sys.exit(2)
            ...
            try:
                ...
                print("ERROR: Flag '-b'|'--bound_violation' set, but reference ",
                      "motion file does not contain Excite_Max")
                sys.exit(2)
        ...
        for i, ref_file in enumerate(reduced_reference_files):
            lns = plot_reference_excitations((axs[i] if num_plots > 1 else axs),
                                             ref_file, resolution, cycle_duration,
                                             lower_dim, upper_dim,
                                             ("Reference Motion-%d" % (i+1)))
        ...
        if learned_actions_file is not None:
            ...
            lns = plot_extracted_excitations(axs[plot_pos], learned_actions_file,
                                             basis_inv, num_frames_to_plot,
                                             resolution, cycle_duration, mean,
                                             lower_dim, upper_dim, root_pos,
                                             root_rot, axis_angle, excite_stats,
                                             animate=animate,
                                             ignore_cycles=ignore_cycles,
                                             offset=offsets[plot_pos],
        ...
        if learned_excitation_file is not None:
            ...
            lns = plot_learned_excitations(axs[plot_pos], learned_excitation_file,
                                           num_frames_to_plot, resolution,
                                           cycle_duration, lower_dim, upper_dim,
                                           excite_stats, animate=animate,
        ...
        if kin_char_extn_file is not None:
            ...
            lns, kin_extn_mat = \
                plot_kin_char_excitations(axs[plot_pos], kin_char_extn_file,
                                          num_frames_to_plot, resolution,
                                          animate, cycle_duration, lower_dim,
                                          upper_dim, offset=offsets[plot_pos],
                                          dtype=reduced_ref_extn.dtype)
            num_legend = max(num_legend, len(lns))
        ...
        for pose_file in pose_files:
            ...
            lns = plot_extracted_excitations(axs[plot_pos], pose_file, basis_inv,
                                             num_frames_to_plot, resolution,
                                             cycle_duration, mean, lower_dim,
                                             upper_dim,  kin_extn_mat, diff_imitation,
                                             root_pos, root_rot, axis_angle=False,
                                             excite_stats=None, animate=animate,
                                             ignore_cycles=ignore_cycles,
                                             offset=offsets[plot_pos],
                                             dtype=reduced_ref_extn.dtype)
            num_legend = max(num_legend, len(lns))
